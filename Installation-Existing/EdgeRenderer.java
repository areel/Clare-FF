...
public final class EdgeRenderer {
    ...
    //FOR MAXFLOW
        if (ctrl.size()%2==0) { //even number of ctrl points - uneven number of edge parts
        	...
        	gc.putText(start.plus(e.thickness*10,e.thickness*10), label, e.thickness*10, e.color);
        } else {
    ...
    private static void drawStraightEdge(Edge e, GralogGraphicsContext gc, boolean isSelected, String label) {
        ...
        //MAXFLOW - Label positioning for parallel edges
        Vector2D center = sourceOffset.plus(intersectionTarget.minus(sourceOffset).multiply(0.5));
        
        // For parallel edges, we need to shift label perpendicular to the edge
        // offset > 0: upper edge → label goes above
        // offset < 0: lower edge → label goes below
        // offset = 0: single edge → label slightly above
        
        double labelDistance;
        if (offset > 0) {
            // Upper edge: push label further away from center line
            labelDistance = Math.abs(offset) + e.thickness * 8;
        } else if (offset < 0) {
            // Lower edge: push label further away from center line (in opposite direction)
            labelDistance = -(Math.abs(offset) + e.thickness * 8);
        } else {
            // Single edge: default small offset upward
            labelDistance = e.thickness * 8;
        }
        
        // Shift perpendicular to edge direction
        Vector2D labelPosition = center.plus(perpendicularToEdge.normalized().multiply(labelDistance));
        
        gc.putText(labelPosition, label, e.thickness*10, e.color);
    }
}